...
import { Lock, Loader2, Wallet, Sparkles, CheckCircle2, ChevronDown, ChevronUp, Clock } from "lucide-react";
...
export default function MessCard() {
  ...
  const expiryDate = cardData?.expiresAt ? new Date(cardData.expiresAt) : null;
  const isExpired = expiryDate ? new Date() > expiryDate : false;
  
  const isInactive = cardData.isActive === "INACTIVE" || isExpired;

  let daysLeft = 0;
  if (!isInactive && expiryDate) {
    daysLeft = Math.ceil((expiryDate.getTime() - new Date().getTime()) / (1000 * 3600 * 24));
  }
  ...
  const handleRecharge = async () => {
    ...
    try {
      ...
      await fetchCardData();
      await refreshUser();
      setToast({ show: true, msg: isExpired ? "Card renewed successfully!" : "Card activated successfully!", type: "success" });
    } catch (error) {
        ...
        <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-gray-900 via-gray-800 to-gray-900 p-6 sm:p-8 text-white shadow-xl border border-gray-700 mb-8">
          ...
          <div className="flex flex-col items-center relative z-10 text-center">
            ...
            <h3 className="text-xl sm:text-2xl font-bold mb-2">
              {isExpired ? "Card Expired." : "Card Inactive."}
            </h3>
            <p className="text-gray-400 text-sm mb-6 max-w-sm mx-auto">
              {isExpired 
                ? "Your 30-day mess pass has expired. Renew now to auto-book your meals." 
            ...
            <button
              ...
              className={`w-full sm:w-auto px-8 py-3.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-lg ${hasEnoughBalance
            ...
            >
              {actionLoading ? (
                ...
                <>
                  ...
                  {isExpired ? "Renew Card" : "Activate Card"} • ₹{PLAN_COST}
                </>
              )}
            </button>
          </div>
    ...
    <div className="mb-2">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          ...
          <div className="flex flex-col items-start sm:items-end gap-2 mt-4 sm:mt-0">
            ...
            {daysLeft > 0 && (
              <span className="text-[10px] font-extrabold bg-white/10 px-3 py-1.5 rounded-md tracking-wider uppercase">
      ...
      >
        <div className="overflow-hidden">
          <div className="pt-2">
            <HostelCalendar expiresAt={cardData.expiresAt} />
          </div>
        </div>
      </div>
